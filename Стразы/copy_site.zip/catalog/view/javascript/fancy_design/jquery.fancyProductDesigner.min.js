...
//		$elem = $(elem).addClass('fpd-container fpd-clearfix fpd-'+options.layout+'').before('<p class="fpd-initiliazing">'+options.labels.initText+'<br /><img src="data:image/svg+xml;base64," /></p>');
		$elem = $(elem).addClass('fpd-container fpd-clearfix fpd-'+options.layout+'').before('<h3 class="fpd-initiliazing2">Пожалуйста, подождите. Идет загрузка конструктора...<br /><img src="/img/ajax-loader.gif" /></h3>');

		$initText = $elem.prev('.fpd-initiliazing2');
		...
		var _init = function() {
			...
			for(var i=0; i < $products.length; ++i) {
				...
				views.each(function(i, view) {
					var $view = $(view);
					var viewObj = {title: view.title, thumbnail: $view.data('thumbnail'), elements: [],id: views.attr('data-parameters'), zdv_category_design_id: $view.data('zdv_category_design_id')};
			...
			if($designs.size() > 0) {
				...
				if($designs.children('.fpd-category').length > 0) {
					...
					$designCategories.chosen();
                                                ...
                                                //zdv
                                                $('#design_category_select').chosen();
                                                $('#design_category_select').change(function(){
                                                    var design_category_id = $(this).val();
                                                    if(design_category_id=='all'){
                                                           $('*[data-zdv_category_design_id]').show();
                                                    }else{
                                                       $('*[data-zdv_category_design_id]').hide();
                                                       $('*[data-zdv_category_design_id="'+design_category_id+'"]').show();
                                                    }
                                                    $('.fpd-items-wrapper').perfectScrollbar('update').scrollTop(0);
                                                });
                                                
                                                $('*[data-zdv_category_design_id]').click(function(){
                                                    $('#product_title_in_menu').html($(this).find('.element-product-design').attr('title'))
                                                });
			...
			if(options.resettable) {
				$menubar.find('.fpd-reset-product').css('cursor','default').click(function(evt) {
					//evt.preventDefault();
...
//			        object.left = object.params.x * responsiveScale * zoomScale;
			        object.left = (object.params.x * responsiveScale)-60;
...
//console.log(objects)
//console.log('replace: '+params.replace)
//console.log('source: '+params.source)
//console.log('evented: '+element.evented)
...
//				object.params.x = left;
                                object.params.x =left+130;
			}

			if(top != undefined) {
				object.params.y = top-40;
			}
		...
		*/
		this.addProduct = function(views,type) {
			...
			$productItemsWrapper.append('<picture class="'+views[0].id+'" data-zdv_category_design_id="'+views[0].zdv_category_design_id+'"><span class="fpd-loading"></span><img class="'+type+'" src="'+views[0].thumbnail+'" title="'+views[0].title+'" style="display:none;" /></picture>').children('picture:last').click(function(evt) {
				evt.preventDefault();
				var $this = $(this),
...
//		        		icon = this.params.resizable ? resizeIcon : false;
                                        icon = false; //костыль - Скрываем значек ресайза, т.к. в админке не отключалась возможность изменения размера
		        	break;
		        	case 'mtr':
	...
	$.fn.fancyProductDesigner.defaults = {
		...
		defaultText: 'Кликните 2 раза для редактирования', //the default text when an user adds a custom text
