...
int main(){
	...
	uidraw(temp = lnPrintList(head,tail,current,lines_count));
...
void drawEditDialog(int selected, ln data){
 ...
 {' ',200,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,205,202,205,205,205,205,205,205,205,188,' '},
	...
	setcolor(LIGHTGREEN);
	switch (selected) {
		case 1: gotoxy(x + 1, y + 2); cout<<""; gotoxy(x + 33, y + 2); cout<<""; break;
		case 2: gotoxy(x + 1, y + 4); cout<<""; gotoxy(x + 33, y + 4); cout<<""; break;
		case 3: gotoxy(x + 1, y + 7); cout<<""; gotoxy(x + 33, y + 7); cout<<""; break;
		case 4: gotoxy(x + 1, y + 9); cout<<""; gotoxy(x + 33, y + 9); cout<<""; break;
		case 5: gotoxy(x + 1, y + 11); cout<<""; gotoxy(x + 33, y + 11); cout<<""; break;
		case 6: gotoxy(x + 1, y + 13); cout<<""; gotoxy(x + 33, y + 13); cout<<""; break;
		case 7: gotoxy(x + 1, y + 16); cout<<""; gotoxy(x + 33, y + 16); cout<<""; break;
		case 8: gotoxy(x + 1, y + 18); cout<<""; gotoxy(x + 33, y + 18); cout<<""; break;
		case 9:
			if (structChecked(data)) {
				for (i = 0; i < 3; i++){
		        for(int j=0; j<12; j++)
...
ln* lnDeleteLine(ln *temp, ln *head, ln *tail, long current, long &lines_count){
	...
	if (current == 1) { // если элемент первый
		...
		--lines_count;
		return head;
	} else if (current == lines_count) { // если элемент последний
		tail = temp->prev;
		delete temp;
		tail->next = NULL;
		temp = NULL;
...
ln* searchLine(ln *head, long &current){
	...
	for (int i = 0; i < 3; i++){
    for (int j = 0; j < 21; j++)
    { gotoxy(x1 + 2+j, y1 + 1 + i); cout << form[i][j]; }
    cout<<endl;
    }
	gotoxy(x1 + 1, y1 + 4);
    ...
    //if (ln==NULL) cout << "   Элемент не найден.  ";
    //else{
...
char *readPath(void){
	...
	char invite[3][21] = {
		...
		{' ','П','у','т','ь',':',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' '},
		{},
	}, ch, path[21]; strcpy(path, "");



    gotoxy(1,1);
	drawDialog("", x1, y1, x2, y2);//прорисовка окна путь
...
int exportList(ln *head, char *path){
	...
	od.gaps.lects = head->in.gaps.lects;
	od.gaps.pract = head->in.gaps.pract;
	...
	long sum = head->in.marks.excl + head->in.marks.good + head->in.marks.stfy + head->in.marks.bad;
	 ...
	 fprintf(fout2, "!===========!=========!===========================!\n");
	 fprintf(fout2, "! Шифр      ! Средний !          Пропущено        !\n");
	 fprintf(fout2, "! дициплины !   бал   !===========================!\n");
	 fprintf(fout2, "!           !         ! Лекций ! Практик ! Всего  !\n");
	 fprintf(fout2, "!===========!=========!========!=========!========!\n");

	//fprintf(fout2, "!=================================================!\n");
	//fprintf(fout2, "! ?шЇЁ      ! ?Ёхфэшщ !          ?Ёюяє?хэю        !\n");
	//fprintf(fout2, "! фшёЎшяышэ?!   сры   !===========================!\n");
	//fprintf(fout2, "!           !         ! ?хъЎшщ ! ?ЁръЄшъ ! ?ёхую  !\n");
	//fprintf(fout2, "!===========!=========!========!=========!========!\n");

	if (!head->next) {
		fprintf(fout2, "!%6s     !%6.1f   !%6d  !%6d   !%6d  !\n", od.discp_code,
					od.mean_mark/(sum != 0 ? sum : 1), od.gaps.lects, od.gaps.pract, od.gaps.lects + od.gaps.pract);
	} else {
		do {
			if (strcmp(pointer->in.discp_code, pointer->prev->in.discp_code)) {
				od.mean_mark = od.mean_mark / (sum != 0 ? sum : 1);
				od.gaps.overall = od.gaps.lects + od.gaps.pract;

				fprintf(fout2, "!  %6s   !%6.1f   !%6d  !%6d   !%6d  !\n", od.discp_code,
						od.mean_mark, od.gaps.lects, od.gaps.pract, od.gaps.overall);

				od.gaps.lects = od.gaps.pract = od.gaps.overall = 0;
				od.mean_mark = 0.0;
			...
			od.gaps.lects += pointer->in.gaps.lects;
			od.gaps.pract += pointer->in.gaps.pract;
